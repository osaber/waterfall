    ...
    window.onscroll=function(){
        if(checkscrollside()){
            $.each( dataInt.data, function( index, value ){
                var $oPin = $('<div>').addClass('pin').appendTo( $( "#main" ) );//$('<div>')创建一个div
...
function waterfall(){
    ...
    var num = Math.floor( $( window ).width() / iPinW )-1;//每行中能容纳的pin个数【窗口宽度除以一个块框宽度】
    ...
    $aPin.each( function( index, value ){
        ...
        if( index < num ){
            ...
            var minHIndex = $.inArray( minH, pinHArr );//返回MINH在pinHArr中的index
            $( value ).css({
                'position': 'absolute',
                'top': minH + 15,
